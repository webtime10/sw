...
jQuery(document).ready(function($) {
    ...
    $(document).on('click', '.tabs_menu a', function(e) {
        ...
        $mapWrap.find('svg [name]').css('fill', '');
            ...
            if (isMatch) {
                ...
                if (post.regions) {
                    post.regions.forEach(function(regionSlug) {
                        ...
                        $mapWrap.find('svg [name]').each(function() {
                            if (normalizeName($(this).attr('name')) === normSlug) {
                                $(this).css('fill', 'rgb(83, 126, 140)');
                            }
                        });
                    });
                }
        ...
        activeRegions.forEach(function(regionSlug) {
            const $targetPaths = $mapWrap.find('svg [name]').filter(function() {
     ...
     */
    $(document).on('click', 'svg [name]', function() {
        const $mapWrap = $(this).closest('.map').first();
        const normRegionName = normalizeName($(this).attr('name')); 
        if ($(this).css('fill') === 'rgb(83, 126, 140)') {
            showPostForRegion($mapWrap, normRegionName); // Здесь класс active добавится по клику
        }
    });
...
<ul class="tabs_menu">
    ...
    if ( have_rows( 'tabs', 'option' ) ) : 
        ...
        while ( have_rows( 'tabs', 'option' ) ) : the_row();
            ...
            if ( $tab_text ) : ?>
                <li>
                    <a data-name="<?php echo wp_kses_post( $anchor ); ?>" href="#tab<?php echo $count; ?>">
...
<svg baseprofile="tiny" fill="#6f9c76" height="640" stroke="#ffffff" stroke-linecap="round" stroke-linejoin="round" stroke-width=".5" version="1.2" viewbox="0 0 1000 641" width="1000" xmlns="http://www.w3.org/2000/svg">
...
<g id="features">
 ...
 <g id="label_points">
  ...
  <circle class="basel_stadt" cx="374.6" cy="100.4" id="CHBS">
  </circle>
  ...
  <circle class="appenzell_ausserrhoden" cx="712.3" cy="162.7" id="CHAR">
  </circle>
  <circle class="appenzell_innerrhoden" cx="737.3" cy="176" id="CHAI">
  </circle>
 </g>
</svg>
			</div>
